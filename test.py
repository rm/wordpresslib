...
class TestWordPressClient(unittest.TestCase):
    def setUp(self):
        self.wp = wordpresslib.WordPressClient(
                'http://www.site.com/wordpress/xmlrpc.php',
                'username', 'password')
    ...
    def testNewPost(self):
        ...
        self.failUnless(
                result, 'deletePost %d failed, return=%s' % (
                        idNewPost, str(result)))
    ...
    def testGetPostCategories(self):
        ...
        for cat in self.wp.getPostCategories(lastPost.id):
            if i == 0:
                self.failIf(cat.isPrimary != 1,
                            'first post category is not isPrimary')
    ...
    def testEditPost(self):
        ...
        p.description = (
                ...
                u'<br><br><em>Questa \xe8 una stringa unicode modificata</em>')
